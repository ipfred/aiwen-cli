...
- `scripts/install-wizard.js` should follow the reference one-command setup flow: `npx @fredzhang/aiwen-cli@latest install` installs or upgrades the global CLI package and installs the project skills. Interactive configuration may be product-specific, but the install + skills flow must stay aligned.
