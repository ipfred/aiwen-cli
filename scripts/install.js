...
function resolveMirrorUrls(env, archive, version) {
  const binaryPath = `/-/binary/aw-cli/v${version}/${archive}`;
 ...
 * release (2020-04-29) that introduced --ssl-revoke-best-effort.
 *
...
function extractZipWindows(archivePath, destDir) {
  ...
  try {
    const dotnet =
      ...
      "[System.IO.Compression.ZipFile]::ExtractToDirectory($env:AW_CLI_ARCHIVE,$env:AW_CLI_DEST)";
    execFileSync("powershell.exe", [...psOpts, dotnet], { stdio: psStdio, env: psEnv });
  } catch (primaryErr) {
    try {
      const cmdlet =
        "$ErrorActionPreference='Stop';" +
        "Expand-Archive -LiteralPath $env:AW_CLI_ARCHIVE -DestinationPath $env:AW_CLI_DEST -Force";
...
function install() {
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "aw-cli-"));
  const archivePath = path.join(tmpDir, archiveName);

  try {
    // Walk the chain in order; stop at the first success. Default chain:
    // GitHub → derived(npm_config_registry)? → npmmirror.
    let lastErr;
    ...
    const extractedBinary = findExtractedBinary(tmpDir, binaryName);
...
function findExtractedBinary(rootDir, binaryName) {
  const preferred = path.join(rootDir, binaryName);
  if (fs.existsSync(preferred)) return preferred;

  const legacyName = `${NAME}_${VERSION}_${platform}_${arch}${isWindows ? ".exe" : ""}`;
  const candidates = [];
  const stack = [rootDir];
  while (stack.length > 0) {
    ...
    for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) {
        stack.push(fullPath);
      } else if (entry.name === binaryName || entry.name === legacyName) {
  ...
  throw new Error(`Extracted binary not found: expected ${binaryName}`);
}
...
if (require.main === module) {
  ...
  // (with AW_CLI_RUN=1) for other commands that do need the binary.
  const isNpxPostinstall =
    process.env.npm_command === "exec" && !process.env.AW_CLI_RUN;
  ...
  try {
    ...
    console.error(
      ...
      `  npm install -g @fredzhang/aiwen-cli\n\n` +
      `  # 2. Point to a corporate npm mirror that proxies /-/binary/aw-cli/...:\n` +
      `  npm install -g @fredzhang/aiwen-cli --registry=https://your-corp-mirror/`
    );
    process.exit(1);
  }
}

module.exports = { getExpectedChecksum, verifyChecksum, assertAllowedHost, resolveMirrorUrls, curlSupportsSslRevokeBestEffort, isCurlVersionSupported, findExtractedBinary };
